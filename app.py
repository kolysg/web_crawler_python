...
def get_next_target(page):
    # input : string
    # returns the next link in a page starting from the page's link

    start_link = page.find('<a href=')
    # page.find returns a position - number

    # if start link doesn't exist
    if start_link == -1:
        return None, 0
...
# # Here's the code in a more readable way: (thanks to Christina-49) 
# def get_all_links(page): 
#     links = [] 
#     while True: 
#         url, endpos = get_next_target(page) 
